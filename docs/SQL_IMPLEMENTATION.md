...
| Watcom query clauses | yes | split top-level `SELECT`/`FROM`/`WHERE`/`JOIN`/`ON`/`GROUP BY`/`HAVING`/`ORDER BY` clauses, logical predicates and multiline query continuations outside strings/comments/nested parentheses | not object-relevant |
