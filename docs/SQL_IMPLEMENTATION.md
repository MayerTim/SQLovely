...
| Watcom query clauses | yes | split top-level `SELECT`/`FROM`/`WHERE`/`JOIN`/`ON`/`GROUP BY`/`HAVING`/`ORDER BY` clauses, logical predicates, multiline query continuations and split `ORDER BY IF ... ENDIF` expression separators outside strings/comments/nested parentheses | not object-relevant |
