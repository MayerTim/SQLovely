...
export interface IndentationEngineInput {
  readonly sourceLines: readonly string[];
  readonly context: FormattingContext;
  readonly pipeline: FormattingPipeline;
}

export interface IndentationEngineResult {
  readonly lines: readonly string[];
  readonly cancelled: boolean;
}

export function formatLinesWithIndentation(input: IndentationEngineInput): IndentationEngineResult {
  const { sourceLines, context, pipeline } = input;
  const { dialect: activeDialect, options: resolvedOptions, indentString } = context;
  ...
  for (const sourceLine of sourceLines) {
    if (isFormattingCancellationRequested(context)) {
      return { lines: formattedLines, cancelled: true };
    }

    const expandedLines = pipeline.expandLine(sourceLine);
  ...
  return { lines: formattedLines, cancelled: false };
}
